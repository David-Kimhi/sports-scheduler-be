...
import z from 'zod';
import { LARGE_L, SMALL_L } from './config.js';
...
async function handleGamesReq(req: Request, res: Response) {
    ...
    const querySchema = z.object({
        sort: z.enum(['date', 'name', 'score']).optional().default('date'),
        direction: z.enum(['asc', 'desc']).optional().default('desc'),
        limit: z.coerce.number().min(1).max(LARGE_L).optional().default(SMALL_L),
        name: z.string().optional().default(''),
        after: z.preprocess(
            (val) => (val ? new Date(val as string) : new Date('1900-01-01')),
            z.date()
        ),
        from: z.preprocess(
    ...
    try {
        const games = await Game.findByName(name, after, sort, direction, limit);
        res.json(games);
      } catch (err: any) {
